...
export function withPathLock(api, path, action) {
    ...
    const handle = api.createFileW(lockPath, abi.GENERIC_READ | abi.GENERIC_WRITE, abi.FILE_SHARE_READ | abi.FILE_SHARE_WRITE, null, abi.OPEN_ALWAYS, 0, null);
    if (isInvalidHandle(handle))
        throwLastError(api, 'CreateFileW', lockPath);
    ...
    if (api.closeHandle(handle) === 0)
        throwLastError(api, 'CloseHandle', `lock file ${lockPath}`);
    return result;
}
/**
...
function readCurrentDacl(api, path) {
    ...
    const readResult = api.getNamedSecurityInfoW(path, abi.SE_FILE_OBJECT, abi.DACL_SECURITY_INFORMATION, ownerSlot, groupSlot, daclSlot, saclSlot, descriptorSlot);
    if (readResult !== abi.ERROR_SUCCESS)
        throwWin32(api, 'GetNamedSecurityInfoW', readResult, path);
...
function mergeAndApply(api, path, entry, oldAcl, descriptor, label) {
    ...
    const applyResult = api.setNamedSecurityInfoW(path, abi.SE_FILE_OBJECT, abi.DACL_SECURITY_INFORMATION, null, null, newAcl, null);
    const freedNew = api.localFree(newAcl);
    if (applyResult !== abi.ERROR_SUCCESS)
        throwWin32(api, 'SetNamedSecurityInfoW', applyResult, `${label}(${path})`);
    if (freedDescriptor !== null && !isNullPtr(freedDescriptor))
        throwLastError(api, 'LocalFree', `${label}(${path}) descriptor`);
    if (!isNullPtr(freedNew))
        throwLastError(api, 'LocalFree', `${label}(${path}) new ACL`);
}
/**
...
function hasExactGrant(oldAcl, sidPtr) {
    ...
    for (let index = 0; index < aceCount; index++) {
        ...
        if (exact && sameSidAt(oldAcl, offset + 8, sidPtr, 0))
            return true;
        offset += aceSize;
    }
    return false;
    ...
    withPathLock(api, path, () => {
        ...
        if (oldAcl !== null && hasExactGrant(oldAcl, sidPtr)) {
            ...
            if (descriptor !== null) {
                ...
                if (!isNullPtr(freed))
                    throwLastError(api, 'LocalFree', `grantWrite(${path}) descriptor`);
            }
            return;
        }
    ...
    return withPathLock(api, path, () => {
        ...
        if (oldAcl === null) {
            if (descriptor !== null) {
                const freed = api.localFree(descriptor);
                if (!isNullPtr(freed))
                    throwLastError(api, 'LocalFree', `revokeWrite(${path}) descriptor`);
            }
            return false;
        }
