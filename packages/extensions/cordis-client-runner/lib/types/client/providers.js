...
export function clientInspectProviders(ctx) {
    return [
        ...
        })),
        {
            ...
            query(method, input) {
                if (method !== 'listSubTree')
                    throw new Error(`unknown Slots inspect method "${method}"`);
                const slots = ctx.get('slots');
                if (slots === undefined)
                    throw new Error('Client Slots service is not running');
        ...
        registration('Theme', 'Current theme token names and light/dark override requirements.', 'listTokens', () => {
            ...
            if (theme === undefined)
                throw new Error('Client Theme service is not running');
