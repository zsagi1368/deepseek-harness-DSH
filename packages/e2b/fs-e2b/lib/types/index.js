...
import { CommandExitError, e2bControlEnvs, FileNotFoundError, FileType, quoteE2BShellArg, } from '@deepseek-ai/dsh-e2b';
...
function decodeCanonicalPath(encoded) {
    ...
    if (!posix.isAbsolute(path))
        throw new Error('fs-e2b: canonical path is not absolute');
    return path;
}
...
export class E2BFileSystem extends FileSystem {
    ...
    fileUrl(target) {
        ...
        if (!posix.isAbsolute(path))
            throw new Error(`fs-e2b: expected an absolute process path: ${JSON.stringify(path)}`);
    ...
    async canonicalPath(sandbox, path, signal) {
        try {
            const result = await sandbox.commands.run(`set -o pipefail; realpath -mz -- ${quoteE2BShellArg(path)} | base64 -w0`, commandOpts(signal));
    ...
    async writeAtomic(target, content, existing, createIfAbsent, signal) {
        ...
        try {
            ...
            if (!created)
                throw new Error('private staging directory already exists');
            ...
            await sandbox.commands.run(`chmod ${mode.toString(8)} -- ${quoteE2BShellArg(temporary)}`, commandOpts(signal));
            assertNotAborted(signal, 'write');
            let committed;
            if (createIfAbsent) {
                ...
                const publication = await sandbox.commands.run(`if ln -T -- ${quoteE2BShellArg(temporary)} ${targetArg}; then printf created; elif test -e ${targetArg} || test -L ${targetArg}; then printf exists; else exit 1; fi`, commandOpts(undefined));
