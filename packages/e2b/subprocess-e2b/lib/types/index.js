...
export class E2BSubprocessRuntime extends SubprocessRuntime {
    ...
    constructor(ctx, config) {
        ...
        ctx.effect(() => async () => {
            ...
            if (failures.length === 1)
                throw asError(failures[0]);
            if (failures.length > 1)
                throw new AggregateError(failures, 'subprocess-e2b: teardown failed');
        }, 'e2b subprocess teardown');
    }
    /** @inheritdoc */
    async resolveExecutable(command, env, signal) {
        if (command.length === 0)
            throw new Error('subprocess-e2b: executable name must be non-empty');
        signal?.throwIfAborted();
        const sandbox = await this.ctx.e2b.getSandbox();
        if (posix.isAbsolute(command)) {
            await sandbox.commands.run(`test -f ${quoteE2BShellArg(command)} -a -x ${quoteE2BShellArg(command)}`, { envs: e2bControlEnvs(), ...signalOpts(signal) });
            signal?.throwIfAborted();
            return command;
        }
        if (command.includes('/')) {
            throw new Error(`subprocess-e2b: command ${JSON.stringify(command)} is a relative path; use an absolute path or a bare PATH name`);
        }
        const path = env?.PATH;
        const prefix = path === undefined ? '' : `PATH=${quoteE2BShellArg(path)} `;
        const result = await sandbox.commands.run(`${prefix}command -v -- ${quoteE2BShellArg(command)}`, { cwd: this.ctx.e2b.cwd, envs: e2bControlEnvs(), ...signalOpts(signal) });
        signal?.throwIfAborted();
    ...
    spawn(spec) {
        if (this.disposing)
            throw new Error('subprocess-e2b: service is disposing');
    ...
    async spawnTerminal(spec) {
        ...
        try {
            const terminal = await spawnE2BTerminal(this.ctx.e2b, { ...spec, signal: setupSignal }, stateDir, this.pollMs);
