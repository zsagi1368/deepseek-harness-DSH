...
export const DEFAULT_LOCKED_SECTIONS = [
 ...
 */
export function checkSectionLock(sectionName, operation, customLocks) {
 ...
 */
export function assertSectionAllowed(sectionName, operation, customLocks) {
    const result = checkSectionLock(sectionName, operation, customLocks);
    if (result.locked) {
        throw new Error(`System prompt section "${sectionName}" is ${result.lockLevel}: ${result.reason}`);
    }
}
/**
 * Get all locked sections for documentation/debugging.
 */
export function getLockedSections(customLocks) {
    return customLocks ?? DEFAULT_LOCKED_SECTIONS;
}
//# sourceMappingURL=locked-sections.js.map