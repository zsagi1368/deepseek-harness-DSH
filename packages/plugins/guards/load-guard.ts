...
import { Plugin, PluginManifest, LoadResult, CheckFailure, CheckWarning } from '../spec/index.js'

interface PreLoadCheck {
  name: string
  run(plugin: Plugin, kernelVersion: string): Promise<CheckResult>
}
...
export class LoadGuard {
  ...
  async preLoad(plugin: Plugin, kernelVersion: string): Promise<LoadResult> {
    const results = await Promise.all(
      this.checks.map(check => check.run(plugin, kernelVersion))
    )

    const failures = results.filter(r => !r.passed).map(r => ({
      check: (r as CheckFailed).message?.split(':')[0] || 'unknown',
      message: (r as CheckFailed).message || 'Unknown error',
      severity: (r as CheckFailed).severity || 'error'
    }))

    const warnings = results
      .filter(r => !r.passed && (r as CheckFailed).severity === 'warning')
      .map(r => ({
        check: (r as CheckFailed).message?.split(':')[0] || 'unknown',
        message: (r as CheckFailed).message || 'Unknown warning'
      }))
...
class VersionCompatibilityCheck implements PreLoadCheck {
  ...
  async run(plugin: Plugin, kernelVersion: string): Promise<CheckResult> {
