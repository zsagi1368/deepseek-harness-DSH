...
import { IconDislikeOutline16, IconLikeOutline16, Tooltip, useAnchoredPosition, } from '@deepseek-ai/dsh-client-ui-primitives';
...
export function MessageFeedbackActions({ messageId, ensure, rate, toggle, clearNote, useFeedback, t }) {
    ...
    const [rowFailure, setRowFailure] = useState(null);
    ...
    const [noteFailure, setNoteFailure] = useState(null);
    ...
    const settleRating = useCallback((result) => {
        if (!alive.current)
            return;
        setPending(false);
    ...
    return (_jsxs(_Fragment, { children: [_jsx(Tooltip, { label: likeLabel, side: "bottom", children: _jsx("button", { type: "button", className: css.action, "aria-label": likeLabel, "aria-pressed": rating === 'positive', "data-active": rating === 'positive' || undefined, disabled: pending, onFocus: seed, onPointerEnter: seed, onClick: () => { onRate('positive'); }, children: _jsx(IconLikeOutline16, {}) }) }), _jsx(Tooltip, { label: dislikeLabel, side: "bottom", children: _jsx("button", { type: "button", className: css.action, "aria-label": dislikeLabel, "aria-pressed": rating === 'negative', "data-active": rating === 'negative' || undefined, disabled: pending, onFocus: seed, onPointerEnter: seed, onClick: () => { onRate('negative'); }, children: _jsx(IconDislikeOutline16, {}) }) }), rating !== undefined && (_jsx("button", { ref: triggerRef, type: "button", className: css.noteOpen, "aria-haspopup": "dialog", "aria-expanded": noteOpen, onClick: toggleNote, children: item?.note === undefined ? t('note.open') : item.note })), rowFailure === null && loadFailed && (_jsx("span", { className: css.failure, role: "status", children: t('error.load') })), rowFailure !== null && _jsx("span", { className: css.failure, role: "status", children: rowFailure }), !(rating !== undefined && noteOpen) && noteFailure !== null && (_jsx("span", { className: css.failure, role: "status", children: noteFailure })), rating !== undefined && noteOpen && createPortal(_jsxs("div", { ref: panelRef, className: css.notePanel, role: "dialog", "aria-label": t('note.dialog'), style: pos ?? MEASURE_STYLE, children: [_jsx("textarea", { ref: inputRef, className: css.noteInput, "aria-label": t('note.aria'), placeholder: t('note.placeholder'), value: draft, rows: 3, onChange: (event) => { setDraft(event.target.value); } }), _jsxs("div", { className: css.noteActions, children: [_jsx("button", { type: "button", className: css.noteSave, disabled: pending, onClick: () => { onSaveNote(rating); }, children: t('note.save') }), _jsx("button", { type: "button", className: css.noteCancel, onClick: closeNote, children: t('note.cancel') })] }), noteFailure !== null && _jsx("span", { className: css.failure, role: "status", children: noteFailure })] }), document.body)] }));
}
