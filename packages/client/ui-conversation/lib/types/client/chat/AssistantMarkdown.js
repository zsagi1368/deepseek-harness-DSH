...
export const AssistantMarkdown = memo(function AssistantMarkdown({ blocks, streaming, interrupted, renderMessageImages, mentions, t, }) {
    ...
    for (let i = 0; i < blocks.length; i++) {
        ...
        switch (block.kind) {
            case 'text':
                rendered.push(_jsx(MarkdownText, { text: block.text, streaming: streaming, codeLabels: codeLabels, fileMentions: mentions }, i));
                break;
            case 'reasoning':
                rendered.push(_jsx(ReasoningRow, { text: block.text, running: streaming && i === last, t: t }, i));
                break;
            case 'image': {
